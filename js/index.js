...
let add = (polish, english) => {
  ...
  if (!tab.some((el) => el.englishWord === english && tab.length > 0)) {
    tab.push({ polishWord: polish, englishWord: english });
    addTab(polish, english);
    createElementTable();
...
let addTab = (polish, english) => {
  tabCopy.push({ polishWord: polish, englishWord: english });
};
